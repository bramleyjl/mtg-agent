...
| Reference decklists (other people's decks, e.g. linked from strategy articles) | 🔧 Ingestion built 2026-07-07, not yet tied to a source article | `reference_decklists` | Renamed from "Other people's decklists" — see dedicated section below |
...
### Reference decklists — ingestion built 2026-07-07, source-article linking pending

**First working example:** Wilson, Refined Grizzly // Noble Heritage, linked from [airza.net's "Ursine Madness"](https://airza.net/2023/08/20/ursine-madness) (two Moxfield decks: a budget build and an optimized build). Chosen as the concrete test case before shaping the `primer`/`strategy_article` CW category — decklist ingestion first, article text ingestion later.
...
**Own vs. reference — decided by comparing owner username, not a manual toggle (2026-07-07).** `MOXFIELD_USERNAME` (new env var, see `CLAUDE.md`) holds John's own Moxfield username. `moxfield.extract_owner_username(deck_data)` reads the authenticated deck payload's `createdByUser.userName` (falling back to `displayName`) — **field name unverified against a real payload as of this writing**, since Moxfield's block prevented fetching a live sample outside the extension; first real extension sync will confirm or require a field-name fix. `/sync-deck` treats a deck as John's own if it's already known (in `decks.yaml` or previously synced into `decks`) *regardless* of the username check — this is the safety fallback if `MOXFIELD_USERNAME` is unset or the field name turns out wrong, so existing own-deck syncing can't silently break. Only decks that are both unknown *and* owned by a different username route to `reference_decklists`.
...
**Source-article linking — built, but only manually populated so far.** The extension's popup gained an optional "source URL / note" text field; whatever's typed there is sent as `source_url` in the `/sync-deck` POST body and stored verbatim. No automatic capture (the extension only sees the Moxfield tab, not whatever tab the article was read in) — John pastes the article URL by hand when syncing a deck he found via one. This is the concrete hook the `strategy_article`/`primer` CW categories will join against once article text ingestion is built.

**Not yet built:** any MCP read tool over `reference_decklists` (e.g. "what decks did this article link" or "how does my Breya list compare to this reference build") — deliberately deferred until the ingestion path above is validated against a real sync (John needs to actually run the extension against the two airza.net Moxfield links to confirm `createdByUser` resolves correctly and the two decks land in Mongo as expected).
